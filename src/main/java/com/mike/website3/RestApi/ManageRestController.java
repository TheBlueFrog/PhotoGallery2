...
public class ManageRestController {
    ...
    private static final String TAG = ManageRestController.class.getSimpleName();


    //            var url = "manage-api/update?imageId=" + imageId + "&<something>=" + value;
    @RequestMapping(value = "/update", method = RequestMethod.GET, produces = "application/text")
    public String get3(HttpServletRequest request, Model model) {
        try {
            String imageId = request.getParameter("imageId");
            Image image = Image.findById(imageId);

            String value = request.getParameter("caption");
            if (value != null) {
                image.setCaption(value);
                image.save();
            }
            value = request.getParameter("public");
            if (value != null) {
                image.setPublic(Boolean.parseBoolean(value));
                image.save();
            }
