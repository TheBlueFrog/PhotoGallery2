...
//            sdf.setTimeZone(TimeZone.getTimeZone("UTC"));
    ...
    public static List<User> sortByComapanyName(List<User> users) {
        Collections.sort(users, new Comparator<User>() {
            @Override
            public int compare(User o1, User o2) {
    ...
    public static List<Item> sortItemsByShortOne(List<Item> items) {
        List<Item> i = new ArrayList<>(items);
        Collections.sort(i, new Comparator<Item>() {
            @Override
            public int compare(Item o1, Item o2) {
                return o1.getShortOne().compareTo(o2.getShortOne());
            }
        });
        return i;
    }
    public static List<UserNote> sortNotesByUserName(List<UserNote> notes) {
        List<UserNote> i = new ArrayList<>(notes);
        Collections.sort(i, new Comparator<UserNote>() {
            @Override
            public int compare(UserNote o1, UserNote o2) {
                // not all records have userIds
                String aName = "";
                String bName = "";
                if (o1.getUserId().length() > 0) {
                    Address a = Address.findNewestByUserIdAndUsage(o1.getUserId(), Address.Usage.Default);
                    aName = a.getLastName() + " " + a.getFirstName();
                }
                if (o2.getUserId().length() > 0) {
                    Address a = Address.findNewestByUserIdAndUsage(o2.getUserId(), Address.Usage.Default);
                    bName = a.getLastName() + " " + a.getFirstName();
                }
...
//                    return o1.getTimestamp().compareTo(o2.getTimestamp());
    ...
    public static List<UserNote> sortNotesByColor(List<UserNote> notes) {
        List<UserNote> i = new ArrayList<>(notes);
        Collections.sort(i, new Comparator<UserNote>() {
            @Override
            public int compare(UserNote o1, UserNote o2) {
                // not all records have userIds
                UserNote.Color aName = o1.getColor();
                UserNote.Color bName = o2.getColor();

                if (aName.equals(bName))
                    return o1.getTimestamp().compareTo(o2.getTimestamp());

                return ((Integer) aName.ordinal()).compareTo((Integer) bName.ordinal());
            }
        });
        return i;
    }

    public static void sortByCompanyName(List<User> users) {
        Collections.sort(users, new Comparator<User>() {
            @Override
            public int compare(User o1, User o2) {
                String coName1 = o1.getCompanyName();
                String coName2 = o2.getCompanyName();
